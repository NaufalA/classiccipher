...
// CrossProductMod is
func (m *Matrix) CrossProductMod(mod int, m2 *Matrix) (*Matrix, error) {
	if mod < 2 {
		return nil, fmt.Errorf("got modulo < 2")
	}
	if m.order != m2.order {
		return nil, fmt.Errorf("matrix orders are not equal")
	}
	var is []int
	for i := 0; i < m.order; i++ {
		for j := 0; j < m.order; j++ {
			t := 0
			for k := 0; k < m.order; k++ {
				t = (t + (m.data[i][k] * m2.data[k][j])) % mod
				if t < 0 {
					t = (t + mod) % mod
				}
			}
